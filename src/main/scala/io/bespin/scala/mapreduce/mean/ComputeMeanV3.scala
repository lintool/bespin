...
class ComputeMeanV3Conf(args: Seq[String]) extends ScallopConf(args) {
...
object ComputeMeanV3 extends Configured with Tool with WritableConversions with Tokenizer {
  ...
  class MyMapper extends Mapper[Text, Text, Text, PairOfLongs] {
    override def map(key: Text, value: Text,
      ...
      context.write(key, new PairOfLongs(value.toString.toLong, 1L))
    }
  }
  ...
  override def run(argv: Array[String]) : Int = {
    val args = new ComputeMeanV3Conf(argv)
